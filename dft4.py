...
class StockTradingEnv(gym.Env):
    ...
    def reset(self):
        ...
        # --- CHANGE 1: Add tracking for peak equity and drawdown ---
    ...
    def _get_obs(self):
        ...
        return np.array([
            ...
            self.peak_equity / self.init_cash # Agent sees its own peak performance
        ], dtype=np.float32)
    ...
    def step(self, action):
        ...
        # --- REWARD SYSTEM WITH DRAWDOWN PENALTY ---
        equity = self.cash + self.stock_owned * price_raw
        
        # 1. Update the peak equity
        self.peak_equity = max(self.peak_equity, equity)
        
        # 2. Calculate drawdown from the peak
        drawdown = (self.peak_equity - equity) / self.peak_equity
        
        # 3. Core reward is the change in equity
        reward = (equity - self.prev_total_asset) / self.init_cash
        
        # 4. Apply a strong penalty for drawdown
        reward -= drawdown * 0.5 # The larger the drawdown, the larger the penalty
        
        # 5. Penalty for using leverage
        if self.cash < 0:
...
model.save("ppo_recurrent_stock_trader_v18")
...
model = RecurrentPPO.load("ppo_recurrent_stock_trader_v18")
