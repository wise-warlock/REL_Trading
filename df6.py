...
class StockTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action == 0: pass
            ...
            if current_stage == 2: leverage = min(leverage, 2.0)
            elif current_stage == 3: leverage = 1.0
        ...
        if current_stage == 1:
            ...
            if current_stage == 2: reward -= drawdown * 0.5
            else: reward -= drawdown * 1.0
...
class LoggerCallback(BaseCallback):
    ...
    def _on_step(self):
        if self.num_timesteps - self.last_step >= 5000:
            ...
            self.ax.set_title("Live Training Equity Curve") # Changed Title
            self.ax.grid(True)
...
# --- CHANGE 1: New function to plot the investment backtest ---
...
FINAL_MODEL_PATH = "ppo_recurrent_stock_trader_v22_final.zip"
...
# --- CHANGE 2: Lists to store data for plotting ---
dates = []
portfolio_values = []
buy_signals = []
sell_signals = []
...
while not done:
    ...
    # --- CHANGE 3: Record data for plotting ---
    dates.append(pd.to_datetime(date))
    portfolio_values.append(portfolio_value)
    
    action_idx = action
    action_str = "HOLD"
    current_buy_signal = np.nan
    current_sell_signal = np.nan
    
    if 1 <= action_idx <= 5 or 11 <= action_idx <= 15: # Any BUY action
        action_str = "BUY"
        current_buy_signal = price
    elif 6 <= action_idx <= 10: # Any SELL action
        action_str = "SELL"
...
# --- CHANGE 4: Call the plotting function ---
