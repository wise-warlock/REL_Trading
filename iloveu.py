...
class StockTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        last_position = self.stock_owned

        # --- ACTION LOGIC ---
        # Action 0: HOLD
        if action == 0:
            pass
        # Actions 1-5: BUY or COVER SHORT
        elif 1 <= action <= 5:
        ...
        # --- REWARD SYSTEM ---
        # 1. Direct reward/penalty for realized Profit and Loss (PnL)
        # This occurs when closing or reducing a position
        if (last_position > 0 and self.stock_owned < last_position) or \
           (last_position < 0 and self.stock_owned > last_position):
            realized_pnl = (price_raw - self.cost_basis) * (last_position - self.stock_owned)
            reward += realized_pnl / self.init_cash # Normalize by initial capital
        
        # 2. Penalty for holding a losing position
        if self.stock_owned != 0:
            unrealized_pnl = (price_raw - self.cost_basis) * self.stock_owned
            if unrealized_pnl < 0:
                reward += unrealized_pnl * 0.001 / self.init_cash # Small continuous penalty

        # 3. Penalty for using leverage
        if self.cash < 0:
            reward -= 0.005

        # Update cost basis if position is opened or increased
        if (last_position == 0 and self.stock_owned != 0) or \
           (last_position > 0 and self.stock_owned > last_position) or \
           (last_position < 0 and self.stock_owned < last_position):
...
model.save("ppo_recurrent_stock_trader_v12")
...
model = RecurrentPPO.load("ppo_recurrent_stock_trader_v12")
